        ...
        <div class="jobsearch-plugin-default-container">
            <div class="jobsearch-row">
                ...
                while (have_posts()) : the_post();
					...
					// CHS Registro Rápido INI para tomar el valor de lo que hay en options jobserach panel admin de que tipo de proyecto se puede aplicar sin login
					
					$apply_without_login_chs = isset($jobsearch_plugin_options['job-apply-without-login']) ? $jobsearch_plugin_options['job-apply-without-login'] : '';
					
					$job_types_actual = wp_get_post_terms($job_id, 'jobtype');
					$allow_types_job = isset($jobsearch_plugin_options['jobtype_allow_without_login']) && !empty($jobsearch_plugin_options['jobtype_allow_without_login']) ? $jobsearch_plugin_options['jobtype_allow_without_login'] : array();

					foreach ($job_types_actual as $job_type) {
                		$job_type_chs = $job_type->slug;
                	if (is_array($allow_types_job) && in_array($job_type_chs, $allow_types_job)) {
                    	$allow_job = true;
                  		}
                 	 }
					
                    // CHS Registro Rápido FIN
                    ...
                    <div class="widget widget_apply_job">
                        <div class="widget_apply_job_wrap">
							...
							if ($application_deadline != '' && $application_deadline <= $current_date) {
                                ...
                                <?php
                            } else if (($apply_without_login_chs == 'on') && (!is_user_logged_in()) && ($allow_job == false)) { // CHS Registro Rápido INI
                                ?>
                                <span class="deadline-closed"><?php esc_html_e('To apply for this project you MUST be REGISTERED', 'wp-jobsearch'); ?></span>
                                <?php
                            } else {                                                                                           // CHS Registro Rápido FIN
