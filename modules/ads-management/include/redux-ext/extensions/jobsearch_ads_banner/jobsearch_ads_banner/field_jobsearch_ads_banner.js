...
jQuery('#ad-type').on('change', function () {
    var _this = jQuery(this);
    var img_con = jQuery('#ad-banner-img-field');
    var mg_url_con = jQuery('#ad-banner-img-url-field');
    var url_target_con = jQuery('#ad-banner-url-target-field');
    var adsense_con = jQuery('#ad-banner-adsense-field');
...
jQuery(document).on('change', '.jobsearch-ad-type-select', function () {
    var _this = jQuery(this);
    var _this_id = _this.attr('data-id');

    var img_con = jQuery('#ad-banner-img-field-' + _this_id);
    var mg_url_con = jQuery('#ad-banner-img-url-field-' + _this_id);
    var url_target_con = jQuery('#ad-banner-url-target-field-' + _this_id);
    var adsense_con = jQuery('#ad-banner-adsense-field-' + _this_id);
    if (_this.val() == 'adsense') {
        img_con.hide();
        mg_url_con.hide();
        url_target_con.hide();
...
jQuery(document).on('click', '.update-ad', function () {
    jQuery(this).parents('.ad-item').find('.action-update-con').slideToggle();
});
...
jQuery(document).on('click', '.remove-ad', function () {
    var conf = confirm(jobsearch_plugin_vars.are_you_sure);
    if (conf) {
        jQuery(this).parents('.ad-item').remove();
    }
});
...
jQuery(document).on('click', '.add-banner-to-list', function (e) {
    ...
    var _this = jQuery(this);
    var _this_id = _this.attr('data-id');
    var this_cont = _this.parents('.ad-banner-fields-sec');

    var html_con = jQuery('.ad-banners-list-sec');

    var this_loader = this_cont.find('.loader-box');
    var this_msg_con = this_cont.find('.message-box');

    var banner_title = this_cont.find('input[id="ad-title"]');
    var banner_type = this_cont.find('select[id="ad-type"]');
    var banner_style = this_cont.find('select[id="ad-style"]');
    var banner_img = this_cont.find('input[id="ad-image"]');
    var banner_img_url = this_cont.find('input[id="ad-img-url"]');
    var banner_url_target = this_cont.find('select[id="ad-target"]');
    var banner_adsense_code = this_cont.find('textarea[id="ad-adsense-code"]');
    ...
    if (banner_type.val() == 'adsense') {
        ...
        if (banner_img.val() == '') {
            error = 1;
            var banner_brwse_btn = jQuery('input[name="ad-image"]');
...
jQuery(document).on('click', '.add-ad-group-to-list', function (e) {
    e.preventDefault();
    var _this = jQuery(this);
    var this_cont = _this.parents('.ad-banner-fields-sec');

    var html_con = jQuery('.ad-groups-list-sec');
