...
if (!class_exists('Jobsearch_JobApplicationLoad')) {
    ...
    class Jobsearch_JobApplicationLoad {
                            ...
                            <form id="apply-form-<?php echo absint($rand_num) ?>" method="post">
                                <div class="jobsearch-user-form jobsearch-user-form-coltwo">
                                    <ul class="apply-fields-list"> 
                                        ...
                                        <!-- INI CHS Campos quitados al formulario p Registro rápido
                                        ...
                                        <li> 
                                         ...
                                         FIN CHS Campos quitados al formulario p Registro rápido -->   
                                        
                                        
                                        <!-- <?php do_action('jobsearch_form_custom_fields_load', 0, 'candidate'); ?> Campos quitados al formulario p Registro rápido -->  
        ...
        public function job_apply_without_login() {
            ...
            global $sitepress;                // CHS registro user rápido //
            ...
            if (is_wp_error($create_user)) {
                ...
                if ($candidate_id > 0) {
					...
					// INI CHS Coloca el registro del user en ingles //
                    ...
                    // FIN CHS Coloca el registro del user en ingles //
					...
					/* INI CHS no se usan - registro rápido
					...
					FIN CHS no se usan - registro rápido */
					
					// INI CHS Define tipo el de candidato según jobtype //
					
					$job_types = wp_get_post_terms($job_id, 'jobtype');
					
                    if (!empty($job_types) && $candidate_type != '') {
                    $allow_apply_b = $allow_apply_a = false;
					}
					
                    
                    $allow_types_type_b = isset($jobsearch_plugin_options['candidate_b_allow_types']) && !empty($jobsearch_plugin_options['candidate_b_allow_types']) ? $jobsearch_plugin_options['candidate_b_allow_types'] : array();
                    
                    $allow_types_type_a = isset($jobsearch_plugin_options['candidate_a_allow_types']) && !empty($jobsearch_plugin_options['candidate_a_allow_types']) ? $jobsearch_plugin_options['candidate_a_allow_types'] : array();
                    
             
			     	foreach ($job_types as $job_type) {
                		$job_type_slug = $job_type->slug;
                	if (is_array($allow_types_type_b) && in_array($job_type_slug, $allow_types_type_b)) {
                    	$allow_apply_b = true;
                  		}
                 	 }
					...
					foreach ($job_types as $job_type) {
                		$job_type_slug = $job_type->slug;
                	if (is_array($allow_types_type_a) && in_array($job_type_slug, $allow_types_type_a)) {
                    	$allow_apply_a = true;
                  		}
                 	  }
					  
			
            	    if ($allow_apply_a === true) {
					     update_post_meta($candidate_id, 'jobsearch_field_candidate_typebase', 'type_a');
				  } else {
				  	  update_post_meta($candidate_id, 'jobsearch_field_candidate_typebase', 'type_b');
                      }
				
				 if ($allow_apply_b && $allow_apply_a) {  // Si el jobtype se permite para los dos candidatos, se escoge crear como orquestrator 
			        update_post_meta($candidate_id, 'jobsearch_field_candidate_typebase', 'type_b');
				   }
				
                  // FIN CHS Define tipo el de candidato según jobtype //
